<div>
    <x-wrapper>

        <div class="columns">
            <div class="column is-3 is-hidden-mobile is-hidden-tablet-only">
                <x-home.col-1></x-home.col-1>
            </div>
            <div class="column is-5-desktop is-8-tablet">
                <x-home.col-2></x-home.col-2>
            </div>
            <div class="column is-4-desktop is-4-tablet">
                <x-home.col-3></x-home.col-3>
            </div>
        </div>

        <div class="columns py-4">

            <div class="column is-4 is-flex">

                <div class="box has-text-centered">

                    <figure class="image is-128x128 is-inline-block mb-4">
                        <img src="img/driving-images/on-street-parking/pay-at-machine.jpg" alt="Pay at machine sign" />
                    </figure>

                    <div class="content">
                        <h2 class="title font-headings is-size-6">
                            Driving Lesson discount packages
                        </h2>
                        <p>
                            You can save money by booking a package of lessons. You can pay via bank
                            transfer.
                            <a href="/prices">Find out more about pricing</a>.
                        </p>
                    </div>
                </div>

            </div>

            <div class="column is-4 is-flex">
                <div class="box has-text-centered">
                    <figure class="image is-128x128 is-inline-block mb-4">
                        <img src="img/driving-images/bus-and-cycle-signs/bus-lane-with-times.png"
                            alt="bus lane road sign" />
                    </figure>

                    <div class="content">
                        <h2 class="title font-headings is-size-6">
                            Learn to drive at a time which suits you
                        </h2>
                        <p>
                            I'm available <strong>Monday to Saturdays</strong>. For
                            more information about current availability please get
                            in touch.
                        </p>
                    </div>
                </div>
            </div>

            <div class="column is-4 is-flex">
                <div class="box has-text-centered">
                    <figure class="image is-128x128 is-inline-block mb-4">
                        <img src="img/driving-images/warning-signs/elderly-residents.png"
                            alt="warning sign elderly people" />
                    </figure>

                    <div class="content">
                        <h2 class="title font-headings is-size-6">
                            Experienced <a target="_blank"
                                href="https://www.gov.uk/government/publications/driving-instructor-grades-explained/driving-instructor-grades-explained">
                                grade A</a> driving instructor
                        </h2>
                        <p>
                            I've been a full-time driving instructor for 20
                            years. In that time I've taught hundreds of people
                            how to drive.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </x-wrapper>
</div>
